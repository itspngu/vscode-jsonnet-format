...
export class Formatter {
  private makeFormatterFlags(options: vscode.FormattingOptions): string[] {
    let extConfig = vscode.workspace.getConfiguration("jsonnet-format");
    return [
      "--indent",
      extConfig.indent == 0 ? options.tabSize : extConfig.indent,
      "--max-blank-lines",
      extConfig.maxBlankLines,
      "--string-style",
      extConfig.stringStyle[0],
      "--comment-style",
      extConfig.commentStyle[0],
      extConfig.prettyFieldNames
        ? "--pretty-field-names"
    ...
    return new Promise<string>((resolve, reject) => {
      ...
      p.on("close", (code, signal) => {
        if (code != 0) {
          reject(
            new Error(
              `Non-zero exit value ${code}: ${Buffer.concat(
                stderr_
              ).toString()}`
            )
          );
...
export class JsonnetDocumentFormattingEditProvider
